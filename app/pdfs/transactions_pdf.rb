class TransactionsPdf < Prawn::Document
  ...
  def build_header
    ...
    # Texto al centro
    elements << make_cell(content: "<b>CircAdmin by</b>", inline_format: true, valign: :center, size: 12)
    ...
    text "#{I18n.t("transactions.type.#{@type}", default: @type.titleize)}", size: 12

    if @transactions.any?
      min = @transactions.minimum(:date)&.strftime("%d/%m/%Y")
      max = @transactions.maximum(:date)&.strftime("%d/%m/%Y")
      text "#{I18n.t("transactions.export.range", default: "Rango")}: #{min} - #{max}", size: 12
    end
  end
  ...
  def table_content
    ...
    @transactions.each do |tx|
      table_data << [
        ...
        tx.title,
        "#{currency}#{'%.2f' % tx.amount}",
        I18n.t("transactions.categories.#{tx.category}", default: tx.category.titleize),
        tx.description.to_s.truncate(60),
        tx.user.user_profile&.full_name || tx.user.email
      ]
    end

    table(table_data, header: true, row_colors: [ "F0F8FF", "FFFFFF" ], cell_style: {
      borders: [ :bottom ],
      padding: [ 4, 6, 4, 6 ],
      size: 9
    }) do
  ...
  def footer
    number_pages "<page> / <total>", at: [ bounds.right - 50, 0 ], size: 9

    creation_note = "#{I18n.t("transactions.export.generated_by", default: "Documento generado por CircAdmin")} — #{Time.current.strftime('%d/%m/%Y %H:%M')}"
